...
public class SOA {
	...
	private static float estabilishAngle (float indoorLuminance, float outdoorLuminance) {
		return 1;
	}
	...
	public static void main(String[] args) {
		...
		//assume the outdoor temperature is the same for all the rooms
		...
		for (int i=0; i<events.length; i++) {
			...
			for (int f = 0; f < ff.length; f++) {
				Food food = ff[f];
				int neededQuantity = estabilishQuantityOfFood(food.getLabel(), expectedPeople);
			...
			//set the correct level of servicies and facilities based on the event
			int LMThreshold = 50;
			int MHThreshold = 100;
			int medFreq = 2;
			int highFreq = 3;
			...
			//for all the needed paths I want to obtain the id of the rooms in each one
			for (int s = 0; (s < pathsToRoom.length) && (satisfiedCapacity < expectedPeople); s++) {
			...
			//for each room that has to be considered I have to obtain the correct luminance and temperature level
			for (int k = 0; k < roomsToConsider.size() ; k++ ) {
				
				String roomId = roomsToConsider.get(k);
				
				float desiredTemperature = estabilishDesiredTemperature(event.getDate(), outdoorTemperature, roomId);
				Location l = new Location();
				l.setRoomId(roomId);
				WeatherCondition wc = nc.getWeatherCondition(l);
				float indoorTemperature = acc.getIndoorStatus(roomId).getTemperaure();
				//choose if use natural or artificial climate control system
				if (naturalClimateUsable(desiredTemperature, indoorTemperature, outdoorTemperature, wc.getPollution)) {
					if (nc.openWindow()){
						//TODO: show alert
					}
				} else {
				...
				if (desiredLuminance > indoorLuminance) {
					...
					if (desiredLuminance < outdoorLuminance) {
						//use natural system
						if (rs._windows.get(0).getAngle() == 1) { //blind is up
							//switch on spotlight
							...
							for (Window w : rs._windows) {
								w.setAngle(estabilishAngle(indoorLuminance, outdoorLuminance));
					...
					if (rs._spotlights.get(0).getLuminance() > 0/*the light is switched on*/) {
